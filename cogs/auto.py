...
from config import result_piar
...
class Auto(commands.Cog):
    ...
    async def auto(self, ctx, arg):
        ...
        if len(arg) < 18:
            ...
            embed.description = 'Для **подключения** автопубликации на вашем сервере должно быть **более 10 участников** или вы можете оформить **золотой доступ** к автопубликации на нашем тех. сервере.[Оформить подписку (кликабельно)](https://discord.gg/8BnEfUq99j).'
            embed.set_footer(text= 'Error: 005')
            await ctx.send(embed = embed)
            return
